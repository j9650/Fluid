...
void Kmeans::Recenter(RgbImage* image, Clusters* clusters) {
	...
	std::cout << "Kmeans::Recenter: " << std::endl;
	double sum = 0;
	for (int y = 0; y < image->h; y++) {
		for (int x = 0; x < image->w; x++) {
			sum += (pow(image->pixels[y][x].r - clusters->centroids[image->pixels[y][x].cluster].r, 2) +
...
//for single stread non-STABLE
void KmeansRandom::AssignClusterXS_E(RgbImage* image, Clusters* clusters, int* counter) {

	std::stringstream ss;
	printf("KmeansRandom::AssignClusterXS_E\n");
...
void KmeansRandom::AssignCluster2(RgbImage* image, Clusters* clusters, int num_thread, int thread_id, std::vector<int>* counter) {
	...
	//for (int x = 0; x < image->w*image->h; x+=num_thread) {
	//for (int x = 0; x < image->w*image->h/num_thread; x++) {
	for ((*counter)[thread_id] = 0; (*counter)[thread_id] < image->w*image->h/num_thread; (*counter)[thread_id]++) {
...
void AssignCluster22_threads(KmeansRandom* kr, RgbImage* image, Clusters* clusters, int num_thread, int thread_id,  std::vector<int>* counter) {
	srand((unsigned)std::chrono::system_clock::now().time_since_epoch().count());
	//int num_thread = counter->size();
	std::random_device rdrd;
	for (int x = 0; x < image->w*image->h/num_thread; x++) {
...
//for multi stread non-STABLE
void KmeansRandom::AssignCluster22(RgbImage* image, Clusters* clusters, int num_thread, std::vector<int>* counter) {

	printf("KmeansRandom::AssignCluster22\n");
	std::thread* threads = new std::thread[num_thread-1];
	for(int i=1; i<num_thread; i++) {
		threads[i-1] = std::thread(AssignCluster22_threads, this, image, clusters, num_thread, i, counter);
	}
	srand((unsigned)std::chrono::system_clock::now().time_since_epoch().count());
	//int num_thread = counter->size();
	std::random_device rdrd;
