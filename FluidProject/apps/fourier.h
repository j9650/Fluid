...
#pragma parrot(input, "fft", [1]dataIn)
		...
		auto v0 = v1.init(call_num, 1);
				...
				Guard*       g2 = gs->newGuard("fftCos" + std::to_string(arg), { v0 }, {}, tp__g2, {  });
...
void FftSin(float x, float *s,int *call_num)
{
   float k = -2* PI * x;
   long int i;
std::cout<<"incoming sine k = "<<k<<std::endl;
*s=k;
float t= k;
...
void calcradix(int* indices, Complex* x, Complex* f,int eI,int oI, float fftSin, float fftCos,int *call_num)
